...
let createEquation = function () {
  let num1 = Math.floor(Math.random() * (100 - 10 + 1)) + 10;
  let num2 = Math.floor(Math.random() * (10 - 2 + 1)) + 2;

  let answer1 = (Math.random() * 15).toFixed(1);
  let answer2 = Math.floor(Math.random() * 13);

  allAnswer = [];
  switchAnswer = [];

  answer = eval(num1 / num2).toFixed(1);
